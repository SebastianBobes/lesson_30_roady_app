...
class Roady:
    def __init__(self, departure_city: str, destination_city: str,
                 number_of_persons: int, fuel_usage:float, fuel:str = "benzina", path = 'config.json'):
        with open(path, 'r') as f:
            config = json.loads(f.read())
        self.distance = Distance(departure_city, destination_city)
...
if __name__ == '__main__':
    ...
    print(f"{roady1.calculate_price_per_person()} lei")
